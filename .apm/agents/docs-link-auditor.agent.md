...
1. **Relative links**: every Markdown link and image reference (inline `[...]` links, `![...]` images, HTML `<img src>`, `<a href>`) that is a relative path must resolve to an existing file. Resolve paths relative to the file containing the link. Flag absolute filesystem paths and links into generated output (`site/`).
